...
public class SlackModel {
    ...
    public void verificarRegistro(Registro registro, Integer idUltimoRegistroMySql,
                                  Integer idUltimoRegistroSqlServer,
                                  EspecificacaoMaquina especificacaoMaquina, Mensagem mensagem) {

        Metrica metrica = buscarMetrica(especificacaoMaquina.getFkMaquina());

        verificarCpu(idUltimoRegistroMySql, idUltimoRegistroSqlServer, registro.getCpuUtilizada(), metrica, mensagem);
        verificarRam(idUltimoRegistroMySql, idUltimoRegistroSqlServer, registro.getRamUtilizada(), metrica, especificacaoMaquina, mensagem);
        verificarDisco(idUltimoRegistroMySql, idUltimoRegistroSqlServer, registro.getDiscoDisponivel(), metrica, mensagem);
        verificarUsb(idUltimoRegistroMySql, idUltimoRegistroSqlServer, registro.getQtdDispositivosUsb(), metrica, mensagem);
    }
    ...
    private void verificarCpu(Integer idUltimoRegistroMySql, Integer idUltimoRegistroSqlServer,
                              Double porcentagemDeUsoDaCpu, Metrica metrica, Mensagem mensagem){

        if (porcentagemDeUsoDaCpu >= metrica.getProblemaCpu()){

            enviarMensagem("""
                           ---------------------------
                           PROBLEMA no uso da CPU
                           Valor capturado: %.2f %%
                           Linha: %s
                           Estação: %s 
                           Máquina: %s
                            """.formatted(porcentagemDeUsoDaCpu, mensagem.getNomeLinha(), mensagem.getNomeEstacao(), mensagem.getNomeMaquina()),
                    "#cpu");
            ...
            enviarMensagem("""
                            ---------------------------
                            CUIDADO no uso da CPU
                            Valor capturado: %.2f %%
                            Linha: %s
                            Estação: %s\s
                            Máquina: %s 
                            """.formatted(porcentagemDeUsoDaCpu, mensagem.getNomeLinha(), mensagem.getNomeEstacao(), mensagem.getNomeMaquina()), "#cpu");
    ...
    private void verificarRam(Integer idUltimoRegistroMySql, Integer idUltimoRegistroSqlServer, Double ramUtilizada, Metrica metrica, EspecificacaoMaquina especificacaoMaquina, Mensagem mensagem){
        Double porcentagemRam = ramUtilizada;

        if (porcentagemRam >= metrica.getProblemaRam()){
            enviarMensagem("""
                            ---------------------------
                            PROBLEMA no uso da RAM
                            Valor capturado: %.2f %%
                            Linha: %s
                            Estação: %s\s
                            Máquina: %s 
                            """.formatted(porcentagemRam, mensagem.getNomeLinha(), mensagem.getNomeEstacao(), mensagem.getNomeMaquina()), "#ram");
    ...
    private void verificarDisco(Integer idUltimoRegistroMySql, Integer idUltimoRegistroSqlServer, Double discoDisponivel, Metrica metrica, Mensagem mensagem){
        if (discoDisponivel <= metrica.getProblemaDisco()){
            enviarMensagem("""
                            ---------------------------
                            PROBLEMA no espaço disponível!
                            Valor capturado: %.2f GB
                            Linha: %s
                            Estação: %s\s
                            Máquina: %s 
                            """.formatted(discoDisponivel, mensagem.getNomeLinha(), mensagem.getNomeEstacao(), mensagem.getNomeMaquina()), "#disco");
    ...
    private void verificarUsb(Integer idUltimoRegistroMySql, Integer idUltimoRegistroSqlServer, Integer qtdUsb, Metrica metrica, Mensagem mensagem){
        if (qtdUsb > metrica.getMaxUsb()){
            enviarMensagem("""
                            ---------------------------
                            Limite de dispositivos USB excedido!
                            Valor capturado: %d 
                            Linha: %s
                            Estação: %s\s
                            Máquina: %s 
                            """.formatted(qtdUsb, mensagem.getNomeLinha(), mensagem.getNomeEstacao(), mensagem.getNomeMaquina()), "#usb");
